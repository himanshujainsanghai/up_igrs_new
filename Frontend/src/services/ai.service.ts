...
import { ApiResponse, AIAnalysisRequest, AIAnalysisResponse } from '@/types';
...
export const aiService = {
   ...
   */
  async triggerAnalysis(complaintId: string, request?: AIAnalysisRequest): Promise<AIAnalysisResponse> {
  ...
  async researchRelatedIssues(complaintId: string): Promise<any> {
    const response = await apiClient.post<ApiResponse<any>>(`/ai/complaints/${complaintId}/research`, {});
    return response.data;
  },
  ...
  async findComplaintOfficers(complaintId: string): Promise<any> {
    const response = await apiClient.post<ApiResponse<any>>(`/ai/complaints/${complaintId}/find-officers`, {});
    return response.data;
  },
   ...
   */
  async draftComplaintLetter(complaintId: string, selectedExecutive?: any): Promise<any> {
    const response = await apiClient.post<ApiResponse<any>>(
  ...
  async generateComplaintActions(complaintId: string): Promise<any> {
    const response = await apiClient.post<ApiResponse<any>>(`/ai/complaints/${complaintId}/generate-actions`, {});
    return response.data;
  },
};
