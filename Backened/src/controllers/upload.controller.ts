...
export const uploadImage = async (
  ...
  try {
    ...
    // Validate file type
    if (!uploadService.validateFileType(file.mimetype, uploadService.ALLOWED_IMAGE_TYPES)) {
      throw new ValidationError('Invalid image type. Allowed: JPEG, PNG, GIF, WebP');
    }

    // Validate file size
    if (!uploadService.validateFileSize(file.size, uploadService.MAX_IMAGE_SIZE)) {
      throw new ValidationError(`File size exceeds maximum allowed size (${uploadService.MAX_IMAGE_SIZE / 1024 / 1024}MB)`);
    }
...
export const uploadDocument = async (
  ...
  try {
    ...
    // Validate file type
    if (!uploadService.validateFileType(file.mimetype, uploadService.ALLOWED_DOCUMENT_TYPES)) {
      throw new ValidationError('Invalid document type. Allowed: PDF, DOC, DOCX, XLS, XLSX, TXT');
    }

    // Validate file size
    if (!uploadService.validateFileSize(file.size, uploadService.MAX_DOCUMENT_SIZE)) {
      throw new ValidationError(`File size exceeds maximum allowed size (${uploadService.MAX_DOCUMENT_SIZE / 1024 / 1024}MB)`);
    }
...
export const uploadVideo = async (
  ...
  try {
    ...
    // Validate file type
    if (!uploadService.validateFileType(file.mimetype, uploadService.ALLOWED_VIDEO_TYPES)) {
      throw new ValidationError('Invalid video type. Allowed: MP4, MOV, AVI');
    }

    // Validate file size
    if (!uploadService.validateFileSize(file.size, uploadService.MAX_VIDEO_SIZE)) {
      throw new ValidationError(`File size exceeds maximum allowed size (${uploadService.MAX_VIDEO_SIZE / 1024 / 1024}MB)`);
    }
